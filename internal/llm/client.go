...
const knockoutSystem = `أنت محلل كروي عربي خبير ومتابع لكأس العالم 2026. مهمتك كتابة تعليق تحليلي قصير ودقيق باللغة العربية الفصحى المبسطة عن نتيجة مباراة في الأدوار الإقصائية (خروج المغلوب).
القواعد:
- انتهى دور المجموعات، فلا تذكر ترتيب المجموعات أو نقاطها أو حسابات التأهل من المجموعة إطلاقاً.
- ركّز على أن الفائز يتأهل للدور التالي والخاسر يودّع البطولة، وأبرز أداء المنتخبين والمنتظر في الدور القادم بإيجاز.
- لا تخترع أرقاماً أو نتائج أو أسماء خصوم في الدور القادم.
- اكتب أسماء المنتخبات بالعربية.
- الطول: من جملتين إلى أربع جمل فقط. بلا مقدمات ولا عناوين.`

func (c *Client) AnalyzeKnockoutResult(ctx context.Context, matchLine, round string) (string, error) {
	prompt := fmt.Sprintf("الدور: %s\nالنتيجة النهائية:\n%s\n\nاكتب التحليل.", round, matchLine)
	return c.generate(ctx, knockoutSystem, prompt, 400)
}
